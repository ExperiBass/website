...
const alert = (msg) => {
    return msg
} // override global alert(), terminals dont have such posh features
...
class Terminal {
    ...
    #commandsDef = {
        ...
        help: {
            ...
            async execute({ classThis, args }) {
                ...
                const command =
                    classThis.#commands.get(args[0]) ??
                    classThis.#commands.get(findCommandFromAlias(args[0], classThis.#commands))
                if (!command) {
        ...
        foxfetch: {
            ...
            async execute() {
                let output = `⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠿⠿⠿⠿⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿   guest@experibassmusic.eth
                ⣿⣿⣿⣿⣿⣿⣿⣿⠟⠋⠁⠀⠀⠀⠀⠀⠀⠀⠀⠉⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿   -----------
                ⣿⣿⣿⣿⣿⣿⣿⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢺⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿   OS: FoxOS
    ...
    async #keyDown(ev) {
        ...
        if (ev.key === 'Enter') {
            ...
            try {
                ...
                const errorMessage = `<span class="errname">${e.name}</span>:<br><span class="errmsg">${
                    e.message
                }</span><br><p class="errstack">@${e.stack.replace(/\n/g, '<br>')}</p>`
    ...
    async #handleCommand(input) {
        ...
        const command =
            this.#commands.get(potentialCommand) ??
            this.#commands.get(findCommandFromAlias(potentialCommand, this.#commands))
